...
def _developer_instructions() -> str:
    return (
        ...
        "prefer one Python sandbox call over a long sequence of transformations. "
        "Recency discipline: when a dataset carries data_recency metadata or "
        "disclosure-date columns, prefer the most recently disclosed records, cite "
        "each figure's disclosure date, and never present an older record as the "
        "current state; when substituting the nearest available dataset for an "
        "unavailable metric, choose the freshest disclosed source and state the "
        "substitution together with its as-of date. If "
        ...
        "Staged temporal models that the user defines in conversation (levels, "
        "stages, or phases whose conditions unfold over time) follow date-bound "
        "evaluation: each stage's entry conditions are tested only on that "
        "stage's own occurrence date, and membership already established through "
        "an earlier transition is not revoked because the query date fails an "
        "earlier stage's entry conditions, unless the model explicitly requires "
        "re-screening every day. To classify a past or current date, anchor "
        "backward: locate the most recent transition on or before that date and "
        "verify that transition's conditions on their own dates, using each "
        "date's own as-of data rather than the latest snapshot. Before executing "
        "such a model, check its clauses for same-day contradictions: a stage "
        "condition that cannot hold on its own trigger day by construction, or "
        "clauses that license conflicting readings of the same judgment, must be "
        "surfaced to the user with numbered interpretation choices and a "
        "recommended default instead of being silently executed, and any stage "
        "that would be permanently empty under a literal reading must be "
        "explicitly flagged. When the chain spans more than a few transitions, "
        "replay the stages day by day in the Python sandbox instead of reasoning "
        "across dates in prose. "
        "A-share board fund flow follows the THS taxonomy contract: read the "
        "board-operation guidance in the operation catalog before ranking or "
        "reporting any board fund-flow figure, because net-flow fields there "
        "are not named intuitively. Name the taxonomy level used (同花顺二级行业 "
        "or 同花顺概念板块) in the answer, and answer fund-flow change questions "
        "by comparing adjacent trading dates on the documented net-flow field. "
