...
def build_query_plan_system_prompt(
    ...
    return (
        ...
        "the user omitted the code. Treat every trusted_security block in the message "
        "as the authoritative ts_code for the named security and never substitute a "
        "different code. For a named company without a trusted_security block, "
        "resolve it through a stock_basic universe query filtered to an exact "
        "full-name match on the name field and bind the returned ts_code into the "
        "security-specific operation through a fan-out template. A "
        "security-specific operation can support a "
        ...
        "In financial disclosure language, a company forecasting, guiding, or warning "
        "about results means published management guidance. Treat it as provider-backed "
        "disclosure data when the catalog exposes such an operation. Only classify a "
        "request as model-generated prediction when the user asks the analysis system "
        "itself to estimate an undisclosed future value. "
        ...
        "not describe an output that the plan does not calculate. Set "
        "required_completeness=complete for exhaustive counts, full-universe screens, "
        "and rankings; otherwise use allow_unknown. List every terminal result needed "
        "for the answer in required_result_ids and optional enrichment results in "
        "advisory_result_ids. Never place one result in both lists. "
        ...
        "AnalysisMetric.type enum values. For ranking or aggregating a native provider "
        "field outside that enum, output only analysis_type=field_analysis intent with "
        "operation, params, fields, filters, analysis_field, optional group_by and "
        "aggregations, and optional ranking. Do not generate queries, result_pipeline, "
        "execution_plan, or answer_contract with this intent; the trusted compiler "
        "owns those details. Never force an unrelated typed metric merely because the "
        "user requests a ranking. "
