...
class DeepSeekQueryPlanner:
    ...
    def _downgrade_unexecutable_plan(plan: QueryPlan, prompt: str) -> None:
        ...
        limitation = None
        for query in plan.queries:
            if query.operation in security_specific and not query.params.get("ts_code"):
                limitation = (
                    f"{query.operation} requires ts_code and cannot perform this "
                    "full-market screen."
                )
                break
            ...
            if (
                ...
                break
        if (
            any(query.operation == "stock_basic" for query in plan.queries)
            and any(
                query.operation
                in {
                    "top10_floatholders",
        ...
        if (
            ...
            limitation = (
                "Full-market holder-concentration ranking requires dynamic "
                "security-specific fan-out, which this bounded plan cannot execute."
            )
        coverage_text = " ".join(
