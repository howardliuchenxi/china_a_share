...
def build_query_plan_system_prompt(
    ...
    return (
        ...
        "build one composed result_pipeline. Use the ranking dataset as the pipeline "
        "source; aggregate to one candidate row per ts_code when needed; sort and "
        "limit the candidate cohort before join_fields; then join each additional "
        "metric by stable keys. A joined query used only for display must produce at "
        "most one row per join key. Join before sort only when the joined field itself "
        "defines the requested ranking. The answer_contract must reference the final "
        "pipeline output and include both selection and enrichment fields. "
