...
test("research result page prioritizes the searchable table without a generic chart", async ({ page }) => {
  await page.route("**/api/research/visualizations/research-task?token=viewer-token", (route) => {
    void route.fulfill({
      status: 200,
      contentType: "application/json",
      body: JSON.stringify({
...
test("discovery page supports broad factor selection and fast reset", async ({ page }) => {
  ...
  await expect(page.getByText("可搜索 54 个", { exact: true })).toBeVisible();
  await page.getByRole("button", { name: "全选全部因子" }).click();
  await expect(page.locator(".factor-selector legend strong")).toHaveText("54");
  await expect(page.locator(".factor-checkbox.is-selected")).toHaveCount(54);
