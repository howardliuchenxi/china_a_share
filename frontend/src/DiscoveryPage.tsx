  ...
  return (
    <div className="discovery-page">
      ...
      {topTrainingRule && <section className="results-panel discovery-summary-panel">
        ...
        <p className="research-caveat">验证通过还要求训练与验证窗口的标签缺失最坏情形提升均大于零：规则内缺失结果按全部失败、规则外的可比基准缺失结果按全部成功计算，同时保持规则样本与基准样本的真实重叠关系。用户配置的交易日门槛同时约束原始不同日期数和按事件权重折算的有效日期数；正式显著性另有至少 20 个有效日的固定底线，Student-t 自由度按有效日数向下取整后计算。证券门槛同样同时约束原始不同证券数和有效证券数，防止少数日期或个股贡献绝大多数命中事件。</p>
      </section>}
