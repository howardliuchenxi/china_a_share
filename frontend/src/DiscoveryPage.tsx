  ...
  return (
    <div className="discovery-page">
      ...
      {bestRule && <section className="results-panel discovery-summary-panel">
        ...
        <p className="research-caveat">研究池限定为沪深北六位证券代码，并排除沪市 900xxx 与深市 200xxx B 股。排行榜名次在训练窗口内锁定，以下验证结果未参与重新排序。标记为“验证通过”的规则必须在训练和验证窗口相对基准均为正向提升，并通过验证集 10% FDR。每条规则在两个窗口都必须同时满足事件数、独立交易日和未来标签覆盖率门槛；停牌等原因造成的未来价格缺失会保留在分母中，不会被静默当作不存在。未来收益采用前后时点一致的复权收盘价计算；任一必需交易日的行情或复权因子整批缺失、或任一数据源请求失败时，研究会直接失败。估值接口成功但无记录时仍保留行情标签，对应估值因子按缺失处理。这是事件研究结果，不等同于可直接交易的组合回测；当前尚未计入涨跌停成交约束、手续费和持仓重叠。置信区间取日期聚类 HAC 与独立交易日 score 区间的保守包络，既处理相邻信号共享收益，也防止全胜或全败样本显示虚假确定性；提升检验还计入规则与全样本基准的重叠。FDR 分母包含所有进入盲测的冻结候选，包括因验证样本不足而未入榜的规则。统计关联仍不代表因果关系。</p>
      </section>}
