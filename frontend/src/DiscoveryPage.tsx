    ...
    <div className="discovery-page">
      ...
      <section className="request-panel">
        ...
        <form onSubmit={handleSubmit} className="discovery-form">
          ...
          <div className="discovery-submit-row">
            <p>连续因子搜索训练集 10% / 25% / 50% / 75% / 90% 分位阈值；有限取值不超过 10 个的离散因子会枚举全部实际阈值，避免罕见序列状态被宽分位遗漏。候选按相对可比基准提升的保守 95% 下界排序，同分时依次比较 5% 下行收益和中位收益，避免平均值被单个异常上涨拉高。{discoveryFactorFields.size * 2} 个配对席位会先覆盖所有存在有效候选的因子，再为每个因子覆盖反方向条件，仍有空位时每个因子方向按训练排名补入一个次优阈值；单条件标签覆盖不足时仍可参与交互生成，因为条件交集可能恢复覆盖，但它不能单独入榜，最终交互仍须重新通过全部覆盖门槛。交互生成完成后，训练提升非正或无法抵御标签缺失的规则不会占用预留验证名额，相同训练样本的规则只保留排名最高者。</p>
      ...
      {topTrainingRule && <section className="results-panel discovery-summary-panel">
        ...
        <div className="headline-metrics">
          ...
          <div><small>超过 {percent(topValidationResult!.target_return)} 的概率</small><strong>{topHasOutcomes ? percent(topValidationResult!.win_rate) : "—"}</strong><em>{topHasOutcomes ? `日期 HAC / 证券聚类、${topValidationResult!.effective_trading_day_count.toFixed(1)} 个有效日与 ${topValidationResult!.effective_security_count.toFixed(1)} 个有效证券 score、缺失标签的保守包络：${percent(topValidationResult!.confidence_lower)} – ${percent(topValidationResult!.confidence_upper)}` : "无可观测验证结果，无法估计概率区间"}</em></div>
          <div><small>相对可比全体提升</small><strong className={topHasOutcomes && topHasBaseline ? (topValidationResult!.win_rate_lift >= 0 ? "metric-positive" : "metric-negative") : undefined}>{topHasOutcomes && topHasBaseline ? `${topValidationResult!.win_rate_lift >= 0 ? "+" : ""}${percent(topValidationResult!.win_rate_lift)}` : "—"}</strong><em>{topHasOutcomes && topHasBaseline ? `保守 95% 包络 ${percent(topValidationResult!.lift_confidence_lower)} – ${percent(topValidationResult!.lift_confidence_upper)} · 可比基准命中率 ${percent(topValidationResult!.baseline_win_rate)}（含规则样本，N=${topValidationResult!.baseline_sample_count}）` : "规则或可比基准没有可观测结果，无法估计提升"}</em></div>
        ...
        <p className="research-caveat">研究池限定为沪深北六位证券代码，并排除沪市 900xxx 与深市 200xxx B 股。只有信号日存在官方日线和有效复权因子的证券才会生成事件：信号日停牌等没有可定义收盘信号的证券不会进入当天股票池；信号生成后，未来停牌、退市等造成的结果缺失则会保留在分母中，不会被静默当作不存在。全市场日线、估值和复权接口达到单页行数上限时会自动继续分页；供应商重复返回同一满页或同一交易日出现重复证券时，研究会快速失败，不会把截断或重复横截面用于搜索。排行榜名次在训练窗口内锁定，以下验证结果未参与重新排序；即使训练候选在验证期证据不足，也会保留原名次并明确显示失败原因，不会让后续规则替补上位。标记为“验证通过”的规则必须满足全部证据门槛、在训练和验证窗口相对基准均为正向提升，并通过验证集 10% BY-FDR。每条规则的基准只包含该规则引用因子均为有限值的可比较事件，避免把因子缺失本身误认为阈值规律；规则禁止引用未来收益、未来价格或未来日期字段。每条规则在两个窗口都必须同时满足事件数、独立交易日、独立证券数和未来标签覆盖率门槛，避免单一个股的长期历史被误称为市场规律；未来价格缺失会按缺失结果全部失败或全部成功的边界扩展概率区间。未来收益采用前后时点一致的复权收盘价计算；任一必需交易日的行情或复权因子整批缺失、或任一数据源请求失败时，研究会直接失败。估值接口成功但无记录时仍保留行情标签，对应估值因子按缺失处理。这是事件研究结果，不等同于可直接交易的组合回测；信号日完整行情通常只能在收盘后确认，因此结果不代表能够按同一收盘价成交。当前尚未计入涨跌停成交约束、手续费和持仓重叠，也没有足以计算真实组合最大回撤的逐日持仓净值路径，因此不会伪造回撤值。命中率置信区间取日期 HAC 与证券聚类中较大的标准误、日期和证券集中度分别折算后的 score 区间，以及缺失标签边界的保守包络；相对提升采用同样的双维度标准误，再与规则—基准概率区间差组成保守包络，既处理相邻信号共享收益，也防止同一股票的持续表现、全胜全败或少量结果缺失制造虚假确定性；正式显著性仍由计入规则与可比较样本重叠的提升检验及 BY-FDR 判定。嵌套分位规则共享大量样本，因此 q-value 使用可控制任意依赖候选族的 Benjamini–Yekutieli 谐波惩罚。验证期少于 20 个日期集中度折算有效日时仍可探索，但显著性固定为 p=1，不能通过 FDR。FDR 分母包含所有进入盲测的冻结候选，包括验证证据不足但仍保留展示的规则。统计关联仍不代表因果关系。</p>
        ...
        <p className="research-caveat">“验证通过”采用正向提升的单侧检验并控制 10% BY-FDR；页面中的提升包络则是更保守的双侧 95% 展示区间，并额外纳入 score 与缺失标签边界。两者回答的问题和严格程度不同，q-value 通过不等于保守 95% 包络必然完全高于零，判断规律时应同时阅读。</p>
        <p className="research-caveat">验证通过还要求训练与验证窗口的标签缺失最坏情形提升均大于零：规则内缺失结果按全部失败、规则外的可比基准缺失结果按全部成功计算，同时保持规则样本与基准样本的真实重叠关系。最低标签覆盖门槛同时约束规则命中事件和完整的因子可比基准；若标签缺失同时造成有效样本、日期或证券不足，验证判定会优先显示标签覆盖失败这一根因。用户配置的交易日门槛同时约束原始不同日期数和按事件权重折算的有效日期数；正式显著性另有至少 20 个有效日的固定底线，Student-t 自由度按有效日数向下取整后计算。证券门槛同样同时约束原始不同证券数和有效证券数，防止少数日期或个股贡献绝大多数命中事件。</p>
      </section>}
        ...
        <div className="rule-list">
          {taskStatus.progress.leaderboard.map((hypothesis, index) => {
            ...
            <div className="rule-body">
              ...
              <p>阈值来源：{thresholdSource(hypothesis.threshold_source)}。规则按训练期相对提升的保守 95% 下界完成排名锁定；该下界使用日期 HAC 与证券聚类中较大的标准误，并包络规则—基准 score 区间差，避免把零标准误或单一股票的持续表现误认为确定性。同分时优先选择 5% 下行分位和中位收益更高的规则。随后进入预留验证，验证结果未参与重新排序。</p>
              ...
              {hypothesis.val_result!.sample_count > 0 ? <>
                ...
                <p className="confidence-note">验证集相对可比全体提升保守 95% 包络：{percent(hypothesis.val_result!.lift_confidence_lower)} – {percent(hypothesis.val_result!.lift_confidence_upper)}</p>
