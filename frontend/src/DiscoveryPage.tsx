    ...
    <div className="discovery-page">
      ...
      <section className="request-panel">
        ...
        <form onSubmit={handleSubmit} className="discovery-form">
          ...
          <div className="discovery-submit-row">
            <p>搜索训练集 10% / 25% / 50% / 75% / 90% 分位阈值；候选按“相对基准提升 − 单侧 95% 不确定性惩罚”排序，24 个配对席位会先覆盖所有存在有效候选的因子，再补充反方向条件，相同训练样本的规则只保留排名最高者。</p>
      ...
      {bestRule && <section className="results-panel discovery-summary-panel">
        ...
        <div className="headline-metrics">
          ...
          <div><small>相对可比样本提升</small><strong className={bestRule.val_result!.win_rate_lift >= 0 ? "metric-positive" : "metric-negative"}>{bestRule.val_result!.win_rate_lift >= 0 ? "+" : ""}{percent(bestRule.val_result!.win_rate_lift)}</strong><em>因子可用样本 {percent(bestRule.val_result!.baseline_win_rate)} · N={bestRule.val_result!.baseline_sample_count}</em></div>
