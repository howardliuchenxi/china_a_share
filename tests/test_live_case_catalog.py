...
def test_canonical_catalog_contains_all_live_cases():
    ...
    assert len(catalog.cases) == 114
    assert len({case.id for case in catalog.cases}) == 114
    assert len({case.prompt for case in catalog.cases}) == 114
    assert sum(case.source == "matrix" for case in catalog.cases) == 100
    assert sum(case.source == "reported_regression" for case in catalog.cases) == 14
