...
@pytest.mark.parametrize(
    ...
    [
        (
            "fina_indicator",
            {"period": "20260331"},
            "The operation requires one security code.",
        ),
        (
            "top_list",
        ...
        (
            "daily_basic",
            {"trade_date": "20260723"},
            "Use PE as a proxy for profitability.",
        ),
    ],
)
...
def test_planner_merges_duplicate_valuation_queries_before_dividend_ranking():
    ...
    plan.queries = [
        DataQuery(
            ...
            params={"trade_date": "20260723"},
            fields=["ts_code", "pe", "dv_ratio"],
            purpose="Filter low PE securities.",
            filters=[{"field": "pe", "operator": "lt", "value": 15}],
        ),
        DataQuery(
            query_id="low-pb",
            operation="daily_basic",
            params={"trade_date": "20260723"},
            fields=["ts_code", "pb", "dv_ratio"],
            purpose="Filter low PB securities.",
            filters=[{"field": "pb", "operator": "lt", "value": 2}],
        ),
    ]
    session = FakeSession(
        FakeResponse({"choices": [{"message": {"content": plan.model_dump_json()}}]})
    ...
    result = DeepSeekQueryPlanner("test-key", session=session).plan(
        AnalysisRequest(prompt="筛选低市盈率、低市净率、高股息率的十只股票。"),
        [DataOperation(name="daily_basic", description="Daily valuation data.")],
    )

    assert len(result.queries) == 1
    assert result.queries[0].transform == "top_10_by_dv_ratio"
    assert set(result.queries[0].fields) == {"ts_code", "pe", "pb", "dv_ratio"}
    assert {
        (row_filter.field, row_filter.operator, row_filter.value)
        for row_filter in result.queries[0].filters
    } == {("pe", "lt", 15.0), ("pb", "lt", 2.0)}


@pytest.mark.parametrize(
    ("prompt", "operation", "params", "fields", "expected_limitation"),
    [
        (
            "昨天量比大于3且上涨的股票有哪些？",
            "daily_basic",
            {"trade_date": "20260723"},
            ["ts_code", "volume_ratio", "pct_chg"],
            "does not provide pct_chg",
        ),
        (
            "最近一年上市的新股有哪些，首日涨幅分别是多少？",
            "new_share",
            {"start_date": "20250724", "end_date": "20260724"},
            ["ts_code", "name", "pct_chg"],
            "does not provide first-day price change",
        ),
        (
            "最近五个交易日融资余额连续增加的股票有哪些？",
            "margin_detail",
            {"trade_date": "${q1:latest_open_date}"},
            ["ts_code", "rzye"],
            "unresolved provider parameter",
        ),
    ],
)
def test_planner_downgrades_known_cross_query_or_placeholder_plans(
    prompt,
    operation,
    params,
    fields,
...
def test_planner_rejects_unimplemented_cross_operation_intersection():
    plan = make_daily_plan()
    plan.queries = [
        DataQuery(
            query_id="volume-ratio",
            operation="daily_basic",
            params={"trade_date": "20260723"},
            fields=["ts_code", "volume_ratio"],
            purpose="Filter high volume ratios.",
        ),
        DataQuery(
            query_id="advancing",
            operation="daily",
            ...
            purpose="Filter advancing securities.",
        ),
    ]
    session = FakeSession(
        FakeResponse({"choices": [{"message": {"content": plan.model_dump_json()}}]})
    ...
    result = DeepSeekQueryPlanner("test-key", session=session).plan(
        AnalysisRequest(prompt="昨天量比大于3且上涨的股票有哪些？"),
        [
            DataOperation(name="daily", description="Daily prices."),
            DataOperation(name="daily_basic", description="Daily valuation data."),
        ],
...
def test_planner_uses_completed_trade_date_for_current_stock_st_query():
    ...
    while completed_date.weekday() >= 5:
        completed_date -= timedelta(days=1)
    plan = make_daily_plan()
    plan.queries[0].operation = "stock_st"
    plan.queries[0].params = {"end_date": now.strftime("%Y%m%d")}
    plan.queries[0].fields = ["ts_code", "name", "st_type", "status"]
    plan.queries[0].filters = [
        DataFilter(field="status", operator="eq", value="ST")
    ]
    session = FakeSession(
        FakeResponse({"choices": [{"message": {"content": plan.model_dump_json()}}]})
    )
    ...
    result = DeepSeekQueryPlanner("test-key", session=session).plan(
        AnalysisRequest(prompt="当前ST股票有哪些？"),
        [DataOperation(name="stock_st", description="Daily ST status.")],
    )

    assert result.queries[0].params == {
        "trade_date": completed_date.strftime("%Y%m%d")
    }
    assert "status" not in result.queries[0].fields
    assert result.queries[0].filters == []


def test_two_limit_up_probability_uses_consecutive_trading_days():
    provider = FakeMarketDataProvider()
    ...
    limit_result = QueryResult(
        ...
        rows=[
            {"trade_date": "20260701", "ts_code": "000001.SZ", "name": "Alpha"},
            {"trade_date": "20260702", "ts_code": "000001.SZ", "name": "Alpha"},
            {"trade_date": "20260702", "ts_code": "600000.SH", "name": "Beta"},
            {"trade_date": "20260703", "ts_code": "600000.SH", "name": "Beta"},
        ],
        row_count=4,
    )
    daily_rows = [
        {"trade_date": trade_date, "ts_code": code, "pct_chg": pct_chg}
        for trade_date, changes in (
            ("20260701", {"000001.SZ": 10.0, "600000.SH": 1.0}),
            ("20260702", {"000001.SZ": 10.0, "600000.SH": 10.0}),
            ("20260703", {"000001.SZ": 2.5, "600000.SH": 10.0}),
            ("20260706", {"000001.SZ": 0.5, "600000.SH": -1.5}),
        )
        for code, pct_chg in changes.items()
    ]
    ...
    assert result.rows[1]["third_trade_date"] == "20260706"
