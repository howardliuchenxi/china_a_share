...
deploy: check
	...
	CLOUDSDK_PYTHON="$(CLOUDSDK_PYTHON)" "$(GCLOUD)" run jobs deploy "$(ANALYSIS_JOB)" \
		...
		--set-secrets TUSHARE_TOKEN=tushare-token:latest,DEEPSEEK_API_KEY=deepseek-api-key:latest,ZAI_API_KEY=zai-api-key:latest,MASSIVE_API_KEY=massive-api-key:latest,FINNHUB_API_KEY=finnhub-api-key:latest \
		--quiet
